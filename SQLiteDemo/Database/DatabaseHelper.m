    ...
    if(version != _dbVersion){
        @try {
            ...
            if(version == 0){
                ...
                NSAssert(NO, @"Database:%@ can degrade from version %d to version %lu. Please reinstall your app!", [[_dbHandler databasePath] lastPathComponent], version, (unsigned long)_dbVersion);
            }
...
-(void)openDB
{
    ...
    if([_dbHandler open]){
        if ([self initDB]) {
            ...
            NSAssert(NO, @"ERROR: initlize database failed!!!");
        }
    } else {
        [_dbHandler close];
        _dbHandler = nil;
        NSAssert(YES, @"ERROR: open database failed!!!");
    }
}
