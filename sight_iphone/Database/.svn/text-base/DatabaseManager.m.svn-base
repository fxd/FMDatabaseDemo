...
- (void)createDatabase:(FMDatabase *)dbHandler
{
    ...
    SSAssertDebug(creator && [creator conformsToProtocol:@protocol(DatabaseCreator)], @"invalid creator: %@", self.databaseInfo.creatorName);
...
- (DBhandler *)DBHelperWithPath:(NSString *)path
{
    ...
    SSAssertDebug(dic && dic.count > 0, @"invalid user database configuration, check Databases.plist");
...
- (FMDatabase *)databaseWithPath:(NSString *)path
{
    @synchronized([DatabaseManager class]){
        ...
        SSAssertDebug(obj && [obj isKindOfClass:[FMDatabase class]], @"invalid DBHandler:%@", obj);
        return obj;
    }
}
