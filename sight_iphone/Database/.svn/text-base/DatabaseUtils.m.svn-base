...
+ (id) rowSetFromResultSet:(FMResultSet *)set
{
    ...
    SSAssertDebug(rowSet.columnNames.count == columnCount, @"ERROR: column names count not equal to column count.");
    while ([set next]) {
...
{
    SSParameterAssert(table);
    SSParameterAssert(values && values.count > 0);
